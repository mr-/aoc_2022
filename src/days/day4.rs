...
fn parse_line(s: &str) -> Option<SectionPair> {
    let (p1, p2) = match s.split(",").collect::<Vec<&str>>()[..] {
        [a, b] => Some((a, b)),
        _ => None,
    }?;
    ...
    return Some((
        (p11.parse().ok()?, p12.parse().ok()?),
        (p21.parse().ok()?, p22.parse().ok()?),
    ));
}
...
fn parse_file() -> Vec<SectionPair> {
    let contents =
        fs::read_to_string("./input/04.txt").expect("Should have been able to read the file");
    let intervals = contents
        .split("\n")
...
pub fn solution() {
    ...
    let count = assignments
        .clone()
        .into_iter()
        .filter(|x| is_contained(x.clone()))
        .count();
    println!("Sol1 {}", count);

    let count = &assignments
        .into_iter()
        .filter(|x| is_overlap(x.clone()))
        .count();
    println!("Sol2 {}", count);
}
